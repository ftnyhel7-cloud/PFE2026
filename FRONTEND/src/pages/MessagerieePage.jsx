...
/* ── Emoji picker ── */
.ch-emoji-wrap { position:static; flex-shrink:0; }
.ch-emoji-picker { position:fixed; bottom:80px; right:24px; background:var(--surface); border:1px solid var(--border); border-radius:var(--r-md); padding:.6rem; box-shadow:var(--sh-lg); display:grid; grid-template-columns:repeat(8,1fr); gap:2px; width:296px; max-height:260px; overflow-y:auto; z-index:500; animation:fadeUp .15s ease; }
@keyframes fadeUp { from{opacity:0;transform:translateY(6px);}to{opacity:1;transform:translateY(0);} }
.ch-emoji-picker button { background:none; border:none; cursor:pointer; font-size:1.18rem; padding:.3rem; border-radius:var(--r-sm); transition:background .1s; line-height:1; }
...
const Ico = {
  ...
  attach:  <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M21.44 11.05l-9.19 9.19a6 6 0 0 1-8.49-8.49l9.19-9.19a4 4 0 0 1 5.66 5.66l-9.2 9.19a2 2 0 0 1-2.83-2.83l8.49-8.48"/></svg>,
  send:    <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M22 2L11 13M22 2L15 22L11 13M22 2L2 9L11 13" stroke="white" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round"/></svg>,
  down:    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="6 9 12 15 18 9"/></svg>,
  ...
  newconv: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 20h9"/><path d="M16.5 3.5a2.121 2.121 0 0 1 3 3L7 19l-4 1 1-4L16.5 3.5z"/></svg>,
  upload:  <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"/><polyline points="17 8 12 3 7 8"/><line x1="12" y1="3" x2="12" y2="15"/></svg>,
  clock:   <svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round"><circle cx="12" cy="12" r="10"/><path d="M12 6v6l4 2"/></svg>,
};
...
const PAL = ['#7c6ff7','#4da6ff','#059669','#d97706','#ec4899','#8b5cf6','#06b6d4','#f43f5e'];
function aColor(id) {
  let h = 0, s = String(id || '');
...
function Av({ user, size = 42, online = false, cls = '' }) {
  return (
    <div className={`ch-av ${online ? 'on' : ''} ${cls}`}
      style={{ width: size, height: size, background: aColor(user?._id || user?.userId), fontSize: size * 0.28 }}>
...
function parseContent(contenu) {
  ...
  return { text: contenu.slice(0, idx), attachments: contenu.slice(idx + 4).split(', ').filter(Boolean) };
}
...
function Highlighted({ text, query }) {
  ...
  return <>{text.split(re).map((p, i) => re.test(p) ? <mark key={i} className="ch-hl">{p}</mark> : p)}</>;
}
...
export default function MessagerieePage() {
  ...
  useEffect(() => {
    ...
    ch.bind('nouveau_message', (msg) => {
      ...
      setConversations((prev) =>
        prev.map((c) => c.interlocuteur?._id === sid
          ? { ...c, nonLus: (c.nonLus || 0) + (sid === oid ? 0 : 1), dernierMessage: msg }
          : c)
      );
      if (sid !== oid) showToast(`💬 Nouveau message de ${msg.idExpediteur?.prenom || 'quelqu\'un'}`);
    });
    ch.bind('typing', ({ fromUserId }) => {
      if (fromUserId === (convSelected?.interlocuteur?._id || convSelected?.userId)) setIsTyping(true);
    });
    ch.bind('stop_typing', ({ fromUserId }) => {
      if (fromUserId === (convSelected?.interlocuteur?._id || convSelected?.userId)) setIsTyping(false);
    });

    return () => { ch.unbind_all(); p.unsubscribe(`private-chat-${user._id}`); p.disconnect(); };
  ...
  const loadEncadrant = async () => {
    setLoading(true);
    try {
      const [a, b] = await Promise.all([API.get('/messagerie/mes-etudiants'), API.get('/messagerie')]);
      setMesEtudiants(a.data || []);
      setConversations(b.data || []);
      setFavorites((a.data || []).slice(0, 5).map((e) => ({ _id: e.userId, prenom: e.prenom, nom: e.nom, image: e.image })));
    } catch (err) {
  ...
  const loadMessages = async (userId) => {
    setLoadingMsgs(true); setError(''); setMsgSearch(''); setShowSearch(false);
    try {
      const { data } = await API.get(`/messagerie/conversation/${userId}`);
      setMessages(Array.isArray(data) ? data : []);
      setConversations((prev) => prev.map((c) => c.interlocuteur?._id === userId ? { ...c, nonLus: 0 } : c));
      setMesEtudiants((prev) => prev.map((e) => e.userId?.toString() === userId ? { ...e, nonLus: 0 } : e));
    } catch (err) {
  ...
  const handleDd = (userId) => {
    ...
    if (!userId) { setConvSelected(null); return; }
    const e = mesEtudiants.find((e) => e.userId?.toString() === userId);
    if (e) setConvSelected({
      interlocuteur: { _id: e.userId, prenom: e.prenom, nom: e.nom, image: e.image, role: 'ETUDIANT' },
      userId: e.userId, sujetTitre: e.sujetTitre, nonLus: e.nonLus,
    });
  };
  ...
  const handleSend = async () => {
    ...
    setSending(true); setTexte(''); setFichiers([]);
    const contenu = txt + (fichiers.length > 0 ? `\n📎 ${fichiers.map((f) => f.name).join(', ')}` : '');
    const temp = {
      _id: 'tmp-' + Date.now(),
      idExpediteur:  { _id: user?._id, prenom: user?.prenom, nom: user?.nom },
      idDestinataire:{ _id: destId },
      contenu, lu: false, createdAt: new Date().toISOString(), _temp: true,
    };
    ...
    try {
      ...
      setConversations((prev) => {
        ...
        if (ex) return prev.map((c) => c.interlocuteur?._id === destId ? { ...c, dernierMessage: data } : c);
        return [{ interlocuteur: convSelected.interlocuteur, dernierMessage: data, nonLus: 0 }, ...prev];
      });
    } catch (err) {
  ...
  const insertEmoji = (em) => { setTexte((p) => p + em); setShowEmoji(false); inputRef.current?.focus(); };

  /* ── Context menu ── */
  const onCtxMenu = (e, msg) => { e.preventDefault(); setCtxMenu({ x: e.clientX, y: e.clientY, msg }); };
  ...
  const deleteMsg = async () => {
    ...
    } catch { showToast('❌ Impossible de supprimer ce message'); }
  };
  ...
  const archiveConv = async () => {
    const id = convSelected?.interlocuteur?._id || convSelected?.userId;
    if (!id) return;
    try { await API.put(`/messagerie/${id}/archiver`); } catch { /* silencieux */ }
  ...
  const convFiltered = conversations.filter((c) => {
    ...
    const matchFilter =
      ...
      activeFilter === 'non-lus'   ? (c.nonLus || 0) > 0 :
      activeFilter === 'etudiants' ? c.interlocuteur?.role === 'ETUDIANT' : true;
  ...
  if (loading) return (
    <>
      <style>{CSS}</style>
      <div className={`ch-wrap${isDark ? '' : ' light'}`} style={{ alignItems:'center', justifyContent:'center' }}>
        <div style={{ textAlign:'center' }}>
          <div className="ch-spinner" style={{ margin:'0 auto .65rem' }} />
          <p style={{ color:'var(--accent)', fontWeight:700, fontFamily:'Inter,sans-serif', fontSize:'.88rem' }}>
  ...
  return (
    <>
      ...
      <div className={`ch-wrap${isDark ? '' : ' light'}`}>
        ...
        {/* Context menu */}
        {ctxMenu && (
          <div className="ch-ctx" style={{ top: ctxMenu.y, left: ctxMenu.x }} onClick={(e) => e.stopPropagation()}>
            <button className="ch-ctx-btn" onClick={copyMsg}>{Ico.copy} Copier le texte</button>
            {isMine(ctxMenu.msg) && (
              <>
                <div className="ch-ctx-sep" />
                <button className="ch-ctx-btn danger" onClick={deleteMsg}>{Ico.trash} Supprimer</button>
              </>
            )}
          </div>
        )}
        ...
        {/* Overlay mobile */}
        <div className={`ch-mob-ov${sidebarOpen ? ' vis' : ''}`} onClick={() => setSidebarOpen(false)} />
        ...
        <aside className={`ch-sb${sidebarOpen ? ' open' : ''}`}>
          ...
          <div className="ch-sb-head">
            ...
            <div className="ch-sb-acts">
              <button className="ch-ib" title="Nouvelle conversation"
          ...
          {role === 'ENCADRANT' && (
            <div className="ch-search-wrap">
              <div className="ch-search-rel">
                ...
                <input className="ch-search" placeholder="Rechercher…" value={search} onChange={(e) => setSearch(e.target.value)} />
              </div>
            </div>
          )}
          ...
          {role === 'ENCADRANT' && (
            <div className="ch-filters">
              {[
                ...
                <button key={key} className={`ch-ftab${activeFilter === key ? ' act' : ''}`} onClick={() => setActiveFilter(key)}>
                  {label}
                  {key === 'non-lus' && totalNonLus > 0 && <span className="ch-badge" style={{ marginLeft:'.3rem', fontSize:'.55rem' }}>{totalNonLus}</span>}
                </button>
              ))}
            </div>
          )}
          ...
          {favorites.length > 0 && (
            <div className="ch-fav">
              <div className="ch-lbl" style={{ paddingLeft:0, paddingTop:0 }}>Favoris</div>
              <div className="ch-fav-row">
                {favorites.map((fav, i) => (
                  <div key={fav._id || i} className="ch-fav-it"
                    onClick={() => { if (role === 'ENCADRANT') handleDd(fav._id?.toString() || ''); setSidebarOpen(false); }}>
          ...
          {role === 'ENCADRANT' && (
            <div className="ch-dd-wrap">
              <select className="ch-dd" value={etudiantSelId}
                onChange={(e) => { handleDd(e.target.value); setSidebarOpen(false); }}>
                <option value="">— Sélectionner un étudiant —</option>
                {mesEtudiants.map((e) => (
                  <option key={e.userId} value={e.userId}>
                    {e.prenom} {e.nom}{e.matricule ? ` · ${e.matricule}` : ''}{e.nonLus > 0 ? ` 🔴 ${e.nonLus}` : ''}
                  </option>
                ))}
              </select>
            </div>
          ...
          {role === 'ETUDIANT' && monEncadrant && (
            <div className="ch-enc-card">
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <p style={{ fontWeight:700, fontSize:'.84rem', color:'var(--text)', marginBottom:'.1rem' }}>
                  Dr. {monEncadrant.prenom} {monEncadrant.nom}
                </p>
                <p style={{ fontSize:'.7rem', color:'var(--accent)', fontWeight:600, marginBottom:'.06rem' }}>Encadrant PFE</p>
                {monProjet?.idSujet?.titre && (
                  <p style={{ fontSize:'.64rem', color:'var(--text2)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
                    📝 {monProjet.idSujet.titre}
                  </p>
                )}
              </div>
              ...
              <div className="ch-conv-list">
                {convFiltered.length === 0 ? (
                  <div className="ch-sb-empty">
                    ...
                    <p style={{ fontSize:'.77rem' }}>{search ? 'Aucun résultat' : 'Aucune conversation'}</p>
                  </div>
                ) : convFiltered.map((conv, i) => {
                  const autre = conv.interlocuteur;
                  const isAct = (convSelected?.interlocuteur?._id || convSelected?.userId) === autre?._id;
                  const { text: prevTxt } = parseContent(conv.dernierMessage?.contenu);
                  return (
                    <div key={autre?._id || i} className={`ch-conv-it${isAct ? ' act' : ''}`}
                      onClick={() => { setEtudiantSelId(autre?._id || ''); setConvSelected({ interlocuteur: autre, userId: autre?._id, nonLus: 0 }); setSidebarOpen(false); }}>
                      <Av user={autre} size={44} />
                      <div className="ch-conv-info">
                        <div className="ch-conv-r1">
                          <span className="ch-conv-name">{autre?.prenom} {autre?.nom}</span>
                          <span className="ch-conv-time">{fConvTime(conv.dernierMessage?.createdAt)}</span>
                        </div>
                        <div className="ch-conv-r2">
                          <span className={`ch-conv-prev${conv.nonLus > 0 ? ' bold' : ''}`}>{prevTxt || 'Démarrer la conversation'}</span>
          ...
          {role === 'ETUDIANT' && (
            <div className="ch-sb-empty">
              ...
              <p style={{ fontSize:'.77rem', lineHeight:1.65 }}>Conversation privée avec votre encadrant assigné uniquement.</p>
            </div>
          )}
        </aside>
        ...
        <section className="ch-chat"
          ...
          {!convSelected ? (
            <div className="ch-empty">
              ...
              <div className="ch-empty-title">{role === 'ENCADRANT' ? 'Sélectionnez un étudiant' : 'Aucun encadrant assigné'}</div>
              <div className="ch-empty-sub">
                {role === 'ENCADRANT' ? 'Utilisez le menu déroulant ou les favoris pour démarrer une conversation.' : error || "Vous n'avez pas encore de projet PFE affecté."}
              </div>
            </div>
          ) : (
            <>
              ...
              <div className="ch-head">
                <button className="ch-ib ch-mob-btn" onClick={() => setSidebarOpen((v) => !v)} style={{ marginRight:'.2rem' }}>
                  {Ico.menu}
                </button>
                  ...
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    <div className="ch-head-meta">
                      <span className="ch-head-role">
                        {convSelected.interlocuteur?.role === 'ENCADRANT' ? '👨‍🏫 Encadrant' : '🎓 Étudiant'}
                      </span>
                ...
                <div className="ch-head-acts">
                  <div className="ch-online-pill"><div className="ch-online-dot" /> En ligne</div>
                  {/* Infos */}
                  <button className="ch-ib" title="Informations sur l'interlocuteur" onClick={showInfo}>{Ico.info}</button>
                  {/* Recherche dans messages */}
                  <button className={`ch-ib${showSearch ? ' on' : ''}`}
                    onClick={() => { setShowSearch((v) => !v); if (showSearch) setMsgSearch(''); }}
                    title="Rechercher dans les messages">{Ico.search}</button>
                  {/* Actualiser */}
                  <button className="ch-ib" title="Actualiser les messages"
                    onClick={() => { loadMessages(convSelected?.interlocuteur?._id || convSelected?.userId); showToast('🔄 Messages actualisés'); }}>
                    {Ico.refresh}
                  </button>
                  {/* Archiver */}
                  <button className="ch-ib" title="Archiver la conversation" onClick={archiveConv}>{Ico.archive}</button>
                  {/* ☀️🌙 Bouton SVG animé */}
              ...
              {showSearch && (
                <div className="ch-msg-search">
                  <input ref={searchRef} placeholder="Rechercher dans cette conversation…"
                    value={msgSearch} onChange={(e) => setMsgSearch(e.target.value)} />
                  {msgSearch && (
                    <span className="ch-search-count">{filteredMsgs.length} résultat{filteredMsgs.length !== 1 ? 's' : ''}</span>
                  )}
                  <button className="ch-ib" onClick={() => { setShowSearch(false); setMsgSearch(''); }}>{Ico.close}</button>
                </div>
              )}
              ...
              <div ref={msgsRef} className="ch-msgs" onScroll={handleScroll}>
                ...
                {loadingMsgs ? (
                  <div style={{ textAlign:'center', padding:'3rem 1rem' }}>
                    <div className="ch-spinner" style={{ width:28, height:28, margin:'0 auto .5rem' }} />
                    <p style={{ color:'var(--text2)', fontSize:'.8rem' }}>Chargement des messages…</p>
                  </div>
                ) : messages.length === 0 ? (
                  <div style={{ textAlign:'center', padding:'3.5rem 1rem' }}>
                    <div style={{ fontSize:'2.8rem', marginBottom:'.75rem', opacity:.2 }}>👋</div>
                    <p style={{ fontWeight:700, color:'var(--text)', marginBottom:'.35rem', fontSize:'.92rem' }}>Démarrez la conversation</p>
                    <p style={{ fontSize:'.79rem', color:'var(--text2)' }}>Envoyez votre premier message à {convSelected.interlocuteur?.prenom}.</p>
                  </div>
                ) : msgSearch && filteredMsgs.length === 0 ? (
                  <div style={{ textAlign:'center', padding:'3rem 1rem', color:'var(--text2)', fontSize:'.82rem' }}>
                    Aucun message ne contient « {msgSearch} »
                  </div>
                ) : (
                  grouped.map((g, gi) => (
                    <div key={gi}>
                      <div className="ch-date-sep"><span>{g.date}</span></div>
                      {g.msgs.map((msg, mi) => {
                        const mine = isMine(msg);
                        const { text, attachments } = parseContent(msg.contenu);
                        return (
                          <div key={msg._id || mi} className={`ch-row ${mine ? 'sent' : 'recv'}`}
                            onContextMenu={(e) => onCtxMenu(e, msg)}>
                            {!mine && <Av user={convSelected.interlocuteur} size={28} />}
                            <div className={`ch-bub${msg._temp ? '' : ' ch-new'}`} style={{ opacity: msg._temp ? .65 : 1 }}>
                              {text && <div>{msgSearch ? <Highlighted text={text} query={msgSearch} /> : text}</div>}
                              {attachments.map((att, ai) => (
                                <div key={ai} className="ch-att">
                                  <span>{fIcoFromName(att)}</span>
                                  <span style={{ maxWidth:130, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', fontSize:'.72rem' }}>{att}</span>
                                </div>
                              ))}
                              <div className="ch-bub-meta">
                                {Ico.clock} {fTime(msg.createdAt)}
                                {msg._temp && <span style={{ opacity:.5 }}> · envoi…</span>}
                                {mine && !msg._temp && (
                                  <span style={{ color: msg.lu ? 'rgba(255,255,255,.7)' : undefined }}>
                                    {msg.lu ? ' ✓✓' : <span style={{ opacity:.45 }}> ✓</span>}
                                  </span>
                                )}
                              </div>
                            </div>
              ...
              {showScrollBtn && (
                <button className="ch-scroll-btn" onClick={scrollToBottom} title="Aller en bas">{Ico.down}</button>
              )}

              {/* File preview */}
              {fichiers.length > 0 && (
                <div className="ch-fprev">
                  <span style={{ fontSize:'.7rem', color:'var(--text2)', fontWeight:600 }}>Fichiers :</span>
                  {fichiers.map((f, i) => (
                    <div key={i} className="ch-fchip">
                      {f.type.startsWith('image/') ? <img src={URL.createObjectURL(f)} alt="" className="ch-fthumb" /> : <span>{fIcoFromName(f.name)}</span>}
                      <span style={{ maxWidth:100, overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{f.name}</span>
                      <span style={{ fontSize:'.6rem', opacity:.6 }}>{(f.size/1024).toFixed(0)}KB</span>
                      <button className="ch-frm" onClick={() => rmFile(i)}>✕</button>
                    </div>
                  ))}
                </div>
              )}
              ...
              <div className="ch-input-area">
                ...
                <button className="ch-ib" title="Joindre un fichier" onClick={() => fileRef.current?.click()}>
                  {Ico.attach}
                </button>

                <div className="ch-input-wrap">
                  <textarea ref={inputRef} className="ch-input" rows={1}
                    placeholder="Écrire un message… (Shift+Entrée pour nouvelle ligne)"
                    value={texte} onChange={(e) => handleTyping(e.target.value)} onKeyDown={handleKey} />
                  <div ref={emojiRef} className="ch-emoji-wrap">
                    <button className={`ch-inp-btn${showEmoji ? ' on' : ''}`} style={{ fontSize:'1.15rem' }}
                      onClick={() => setShowEmoji((v) => !v)} title="Emojis">😊</button>
                    {showEmoji && (
                ...
                <button className="ch-send" onClick={handleSend}
                  disabled={(!texte.trim() && fichiers.length === 0) || sending} title="Envoyer (Entrée)">
                  {sending
